...
觀測有**兩條來源路徑**，共用 `eventService.appendObservation`：
新回報（batchWorker），以及現場目擊者在確認頁回答「他現在在哪」
（`POST /api/events/:id/confirm` 的 `step: 'sighting'`）。
後者是移動追蹤實際可用的關鍵——只靠前者，等於要求目擊者重走一次完整回報流程
才能說出歹徒位置。目擊回報**不是投票**：同一個人可以回報多次、不計入門檻、
不參與否證，但一律要求已確認在場（軌跡會變成疏散方向建議，不能被遠端污染）。
