...
const routes = [
  ...
  ["POST", /^\/api\/events$/, async (m, req, res, body) => {
    ...
    const payload = { ...body };
    // 語意座標: 前端兩階快選 (space/anchor/level) 或文字抽取
    if (body.space && Geo2.SPACES[body.space]) {
      payload.semantic = Geo2.semanticLocation(body.space, body.anchor, body.level);
      payload.level = body.level || null;
    } else if (!body.semantic) {
      const ext = Geo2.extractSemantic(text);
      if (ext) payload.semantic = ext;
    }
    const ev = Store.create(payload);
    ev.title = AI.titleFrom(body.kind, text);
    ev.spaceKey = body.space || null; // 供引導引擎使用
    if (payload.semantic && !ev.facts.location) {
      ev.facts.location = payload.semantic; // 語意座標直接成為位置事實
      ev.factsTs.location = now();
    }

    // AI Agent 解讀 (類別/嚴重度/事實); 使用者指定的 severity 優先 (create 已採用, AI 為後備)
  ...
  ["POST", /^\/api\/admin\/scenario$/, async (m, req, res, body) => {
    ...
    const SCENARIOS = {
      ...
      mall: {
        ...
        space: "mall", anchor: "ZARA", level: "3F", severity: 2,
        title: "人群 — 百貨 3F 走失",
        sim: [
    ...
    pushSystem(ev, "zone", `AI 指揮官(引導): ${guide.join(" / ")}`);
