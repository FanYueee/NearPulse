...
| 路徑 | 裝置 | 用途 |
|---|---|---|
| `/` | 手機 UA | 自動導向 **`/mobile/`** — 通報端（零打字狀態機 + 語音 + 照片邊緣運算） |
...
### 5. 照片邊緣運算
前端 Canvas 壓縮（800px WebP < 30KB）+ Laplacian 變異數模糊篩檢 + 亮度檢查——黑屏/手指/全糊的廢片在手機端當場攔截，基地台塞車時每筆上傳都是高價值訊號。（`public/mobile/app.js` `compressAndCheck`）
...
| 語音通報 + AI 判讀代填 | 完整 | 前端 SpeechRecognition + `llm-agent.js` |
| 照片邊緣壓縮 + 廢片篩檢 | 完整 | `public/mobile/app.js` |
| 多語語音播報（中/英/日） | 完整 | 前端 SpeechSynthesis |
